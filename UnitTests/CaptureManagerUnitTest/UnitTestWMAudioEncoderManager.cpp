...
#include "../../CaptureManagerSource/EncoderManager/WMAudioEncoderManager.h"
...
namespace CaptureManagerUnitTest
{
	...
	TEST_CLASS(UnitTestWMAudioEncoderManager)
	{
	public:
		...
		TEST_METHOD(enumEncoderMediaTypes)
		{
			...
			CComPtrCustom<IEncoderManager> lWMAudioEncoderManager = new (std::nothrow)WMAudioEncoderManager();
			
			Assert::IsFalse(!lWMAudioEncoderManager, L"lWMAudioEncoderManager is nullptr");

			EncodingSettings lEncodingSettings;

			std::vector<CComPtrCustom<IUnknown>> lListOfMediaTypes;

			auto lresult = lWMAudioEncoderManager->enumEncoderMediaTypes(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMAEncMediaObject),
				lListOfMediaTypes);

			Assert::IsFalse(lresult,
				std::wstring(std::wstring(L"WMAudioEncoderManager: enumEncoderMediaTypes() cannot be executed!!! ") +
				std::wstring(DataParser::getHRESULTDescription(lresult))).c_str()
				);
		...
		TEST_METHOD(getCompressedMediaType)
		{
			...
			auto lresult = lWMAudioEncoderManager->getCompressedMediaType(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMAEncMediaObject),
				0,
		...
		TEST_METHOD(getEncoder)
		{
			...
			auto lresult = lWMAudioEncoderManager->getEncoder(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMAEncMediaObject),
				0,
			...
			Assert::IsFalse(lresult,
				std::wstring(std::wstring(L"WMAudioEncoderManager: getEncoder() cannot be executed!!! ") +
				std::wstring(DataParser::getHRESULTDescription(lresult))).c_str()
				);
