...
#include "../../CaptureManagerSource/EncoderManager/WMVideo9EncoderManager.h"
...
namespace CaptureManagerUnitTest
{
	...
	TEST_CLASS(UnitTestWMVideo9EncoderManager)
	{
	public:
		...
		TEST_METHOD(enumEncoderMediaTypes)
		{
			...
			CComPtrCustom<IEncoderManager> lWMVideo9EncoderManager = new (std::nothrow)WMVideo9EncoderManager();

			Assert::IsFalse(!lWMVideo9EncoderManager, L"lWMVideo9EncoderManager is nullptr");
			...
			auto lresult = lWMVideo9EncoderManager->enumEncoderMediaTypes(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMV9EncMediaObject),
				lListOfMediaTypes);

			Assert::IsFalse(lresult,
				std::wstring(std::wstring(L"WMVideo9EncoderManager: enumEncoderMediaTypes() cannot be executed!!! ") +
				std::wstring(DataParser::getHRESULTDescription(lresult))).c_str()
				);
		...
		TEST_METHOD(getCompressedMediaType)
		{
			...
			auto lresult = lWMVideo9EncoderManager->getCompressedMediaType(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMV9EncMediaObject),
				0,
		...
		TEST_METHOD(getEncoder)
		{
			...
			auto lresult = lWMVideo9EncoderManager->getEncoder(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMV9EncMediaObject),
				0,
			...
			Assert::IsFalse(lresult,
				std::wstring(std::wstring(L"WMVideo9EncoderManager: getEncoder() cannot be executed!!! ") +
				std::wstring(DataParser::getHRESULTDescription(lresult))).c_str()
				);
