...
#include "../../CaptureManagerSource/EncoderManager/WMVideo8EncoderManager.h"
...
namespace CaptureManagerUnitTest
{
	...
	TEST_CLASS(UnitTestWMVideo8EncoderManager)
	{
	public:
		...
		TEST_METHOD(enumEncoderMediaTypes)
		{
			...
			CComPtrCustom<IEncoderManager> lWMVideo8EncoderManager = new (std::nothrow)WMVideo8EncoderManager();

			Assert::IsFalse(!lWMVideo8EncoderManager, L"lWMVideo8EncoderManager is nullptr");
			...
			auto lresult = lWMVideo8EncoderManager->enumEncoderMediaTypes(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMVXEncMediaObject),
				lListOfMediaTypes);
		...
		TEST_METHOD(getCompressedMediaType)
		{
			...
			auto lresult = lWMVideo8EncoderManager->getCompressedMediaType(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMVXEncMediaObject),
				0,
		...
		TEST_METHOD(getEncoder)
		{
			...
			auto lresult = lWMVideo8EncoderManager->getEncoder(
				lInputType,
				lEncodingSettings,
				__uuidof(CWMVXEncMediaObject),
				0,
